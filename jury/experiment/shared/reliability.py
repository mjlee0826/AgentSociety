"""
experiment/shared/reliability.py

共用 Test-retest Reliability 計算基類與 domain-neutral 資料結構。

設計原則：
  PersonaReliabilityBase / ReliabilityStatsBase — domain-neutral 資料類別，
  jury 與 bystander 共用同一套 reliability 資料格式。

  BaseReliabilityCalculator — 封裝「分組 → 計算 modal decision → 計算 agreement
  率 → 按 occupation/OCEAN 切片」的共用核心邏輯。

  jury/reliability.py（ReliabilityCalculator）與
  bystander/reliability.py（SmokeReliabilityCalculator）都繼承此基類，
  各自固定 DecisionSchema（JURY_SCHEMA 或 BYSTANDER_SCHEMA）後對外暴露
  calculate(records, repeat_n) → ReliabilityStatsBase。

  前提：records 需有 .decision、.occupation_category、.ocean_profile、
         .confidence、.persona_id 屬性。
  VotingRecord 透過 @property decision（alias for verdict）符合此介面。

  主要欄位名稱變化（與舊版 reliability.py 相比）：
    - verdict_counts → decision_counts（domain-neutral）
    - modal_verdict  → modal_decision（domain-neutral）
"""
...
@dataclass
class PersonaReliabilityBase:
    """
    單一 persona 在 N 次重複測量下的 reliability 指標。

    domain-neutral：jury 與 bystander 共用。
    decision_counts 以 {decision_value: count} 儲存；
    modal_decision 為出現最多的合法決策值。

    Attributes:
        persona_id:          persona 唯一識別碼。
        occupation:          職業字串。
        occupation_category: 職業類別（legal/medical/…）。
        ocean_key:           五字母 OCEAN 縮寫（如 'lmhlh'）。
        run_count:           實際完成的執行次數。
        decision_counts:     {decision_value: count}（含 error_value）。
        modal_decision:      出現次數最多的決策（優先非 error）。
        modal_count:         modal_decision 的出現次數。
        modal_agreement:     modal_count / run_count；1.0 = 完全一致。
        is_fully_consistent: modal_agreement == 1.0。
        confidence_mean:     N 次 confidence 的平均（排除 parse_error）。
        confidence_sd:       N 次 confidence 的樣本標準差。
    """
...
class ReliabilityStatsBase:
    ...
    Attributes:
        repeat_n:                設計上每 persona 預計重複次數。
        total_personas:          參與重複測量的 persona 數。
        fully_consistent_count:  modal_agreement == 1.0 的 persona 數。
        fully_consistent_rate:   fully_consistent_count / total_personas。
        mean_modal_agreement:    所有 persona modal_agreement 的平均。
        min_modal_agreement:     最不穩定的 persona 的 modal_agreement。
        mean_confidence_sd:      所有 persona confidence_sd 的平均。
        modal_agreement_buckets: modal_agreement 分桶計數。
        per_persona:             詳細逐人指標。
        by_occupation_category:  各職業類別的 reliability 摘要。
        by_ocean_dimension:      各 OCEAN 維度高低的 reliability 摘要。
    """
    ...
    min_modal_agreement:     float
    mean_confidence_sd:      float
    modal_agreement_buckets: dict
    per_persona:             list[PersonaReliabilityBase]
...
class BaseReliabilityCalculator:
    ...
    子類別只需呼叫 calculate(records, repeat_n, schema) 並固定 schema 即可。
    所有計算邏輯皆在此基類中實作。

    Records 需有以下屬性：
      - .decision:            str（決策值）
      - .occupation_category: str（職業類別）
      - .ocean_profile:       dict（OCEAN 維度 → "high"/"medium"/"low"）
    ...
    def calculate(
        ...
        """
        從 N 次重複測量的記錄計算 reliability 統計。

        Args:
            records:  含 run_index 的所有決策記錄；同一 persona_id 應有多筆。
            repeat_n: 實驗設計上每 persona 預計的重複次數。
            schema:   DecisionSchema（決定 error_value 與 valid_choices）。
        ...
        per_persona = [self._calc_persona(rs, schema) for rs in grouped.values()]

        if not per_persona:
            # 空 records 邊界處理；理論上不會發生
            return ReliabilityStatsBase(
                repeat_n=repeat_n,
                total_personas=0,
        ...
        return ReliabilityStatsBase(
            ...
            by_ocean_dimension=self._by_ocean(grouped, schema),
    ...
    def _calc_persona(
        ...
        Args:
            ...
            schema: DecisionSchema（提供 error_value）。
        """
        error_value = schema.error_value

        # 依決策計數（含 parse_error）
        decision_counts = dict(Counter(r.decision for r in runs))

        # 找出 modal decision：忽略 parse_error；若全部都是 error 則退而求其次
        # Modal decision: prefer non-error; if all errors, fall back to error
        non_error = {d: c for d, c in decision_counts.items() if d != error_value}
        if non_error:
            modal_decision, modal_count = max(non_error.items(), key=lambda kv: kv[1])
        else:
            modal_decision, modal_count = max(decision_counts.items(), key=lambda kv: kv[1])
        ...
        valid_conf = [r.confidence for r in runs if r.decision != error_value]
        if valid_conf:
    ...
    def _bucketize(self, per_persona: list[PersonaReliabilityBase]) -> dict:
        """
    ...
    def _by_occupation(self, per_persona: list[PersonaReliabilityBase]) -> dict:
        """按職業類別切片，看是否某些職業更不穩定。"""
        buckets: dict[str, list[PersonaReliabilityBase]] = defaultdict(list)
        for p in per_persona:
    ...
    def _by_ocean(
        ...
        """
        # 先彙整 persona_id → (ocean_profile, runs)
        ...
        for dim in _OCEAN_DIMENSIONS:
            ...
            result[dim] = {
                "high": self._slice_reliability(high_runs, schema),
                "low":  self._slice_reliability(low_runs, schema),
            }
        return result

    def _slice_reliability(self, runs: list, schema: DecisionSchema) -> dict:
        ...
        per_persona = [self._calc_persona(rs, schema) for rs in by_pid.values()]
        n           = len(per_persona)
        return {
            "persona_count":         n,
