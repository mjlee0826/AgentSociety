...
設計原則：
  ...
  關鍵設計：VotingRecord 新增 @property decision（alias for verdict），
  使 BaseStatsCalculator / BaseReliabilityCalculator 可以用統一的 .decision
  屬性同時處理 VotingRecord 與 SmokeDecisionRecord。
"""
...
class VotingRecord:
    ...
    decision（@property）：verdict 的別名，供 BaseStatsCalculator /
              BaseReliabilityCalculator 以統一介面存取，非 breaking change。
    """
    ...
    def decision(self) -> str:
        """
        verdict 的別名，提供統一的 .decision 介面。

        供 BaseStatsCalculator / BaseReliabilityCalculator 以統一方式存取
        jury 與 bystander 的決策欄位（兩者欄位名稱不同）。

        Alias for verdict — unified interface for BaseStatsCalculator /
        BaseReliabilityCalculator, allowing both VotingRecord (jury) and
        SmokeDecisionRecord (bystander) to be processed by the same base classes.
        """
        return self.verdict
